...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['select_customer'])) {
    ...
    $stmt = $conn->prepare("
        SELECT cd.*, a.agent_name, MAX(pe.purchase_datetime) AS last_purchase, MAX(CASE WHEN pe.result = 'Win' THEN pe.purchase_datetime ELSE NULL END) AS last_win 
                            ...
                            <table id="purchaseEntriesTable" class="table table-bordered" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($related_purchases as $purchase): ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $purchase['result'] ?? 'Pending'; ?></td>
                                            <td>$<?php echo isset($purchase['winning_amount']) ? number_format($purchase['winning_amount'], 2) : 'N/A'; ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                                <!-- Subtotal row -->
                                <tfoot>
                                    <tr>
                                        ...
                                        <td colspan="3"><strong>$<?php echo number_format($subtotal, 2); ?></strong></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
